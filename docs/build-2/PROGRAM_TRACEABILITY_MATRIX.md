...
| Active Week Group has one owner | `domain/week-groups.mjs` | Gate A domain suite | Staged | Gate B canonical config entity |
| Last arrival is derived, not manually recalculated | `domain/arrivals.mjs` | Gate A domain suite | Staged | Status Board migration |
| Manifested bus log is distinct from arrived total | `domain/buses.mjs` | Gate A and operational truth suites | Staged | Airport migration |
...
| Timer/overtime calculation is deterministic | `domain/timers.mjs` | Gate A domain suite | Staged | Status/Processing timer integration |
...
| Build 1 records normalize without silent loss | `data/record-normalizer.mjs` | normalization and parity suites | Staged compatibility boundary | Gate B direct canonical entities |
...
| Transactional Week Group initialization | future workflow orchestrator | future partial-failure/recovery suite | Not implemented | Gate D |
...
| One route and permission registry | `shell/route-registry.mjs`, `permission-registry.mjs` | shell suite | Staged | Server authorization in Gate C/Phase 3 |
...
| Squadron credentials remain server-side | Cloudflare bindings and repository safeguards | repository/config tests; future auth tests | Not activated | Gate C/Phase 3 Squadron migration |
