...
| `#archive-edit-modal` | Legacy archive-edit compatibility markup | No longer opened by the canonical Archives workspace | Instructor only if explicitly invoked by a retained legacy path | Historical correction form fields | Legacy close actions | Retained base-DOM compatibility only. The active Archives workspace is read-only; archive deletion is retired and corrections use the immutable amendment workflow. |
