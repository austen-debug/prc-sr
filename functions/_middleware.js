...
function printArchiveSpreadsheet() {
  ...
  function reportRows(items, builder, colspan) {
    if (!items || items.length === 0) {
      return '<tr><td class="empty-row" colspan="' + colspan + '">No records.</td></tr>';
    }
  ...
  const totalArrived = reportNumber(document.getElementById('archive-edit-total-arrived').value);
  const femaleTotal = reportNumber(document.getElementById('archive-edit-female-total').value);
  const natTotal = reportNumber(document.getElementById('archive-edit-nat-total').value);
  const loadedTotal = dormData.reduce((s, d) => s + reportNumber(d.current_load || d.loaded || 0), 0);
  const expectedTotal = dormData.reduce((s, d) => s + reportNumber(d.max_load || 0), 0);
  const printedAt = new Date().toLocaleString();
  const processedNarrative = 'Today, the PRC processed ' + totalArrived + ' of the ' + expectedTotal + ' projected trainees, for a total of ' + loadedTotal + ' trainees loaded to dorms. ' + natTotal + ' trainees requested U.S. Naturalization, for a total of ' + natTotal + ' naturalization requests.';

  const dormRows = reportRows(dormData, function(d) {
    return '<tr>' +
  ...
  const busRows = reportRows(busData, function(b) {
    return '<tr>' +
      ...
      '<td>' + reportDateTime(b.departed_at) + '</td>' +
      '<td>' + reportDateTime(b.arrived_at) + '</td>' +
  ...
  const html = [
    ...
    '.classification { display:flex; justify-content:center; align-items:center; min-height:28px; background:var(--green); color:#ffffff; font-size:11px; font-weight:900; letter-spacing:0.12em; text-transform:uppercase; border:1px solid #14532d; }',
    '.no-print { margin:12px 0; padding:9px 16px; border:1px solid #94a3b8; border-radius:8px; background:#0f172a; color:#fff; font-weight:800; letter-spacing:0.04em; cursor:pointer; }',
    ...
    '.title-block h1 { margin:0; font-size:28px; letter-spacing:-0.04em; text-transform:uppercase; }',
    '.subtitle { margin-top:4px; color:var(--muted); font-size:12px; font-weight:800; letter-spacing:0.06em; text-transform:uppercase; }',
    '.meta { min-width:255px; border:1px solid var(--line); background:var(--soft); padding:10px; font-size:11px; }',
    '.meta-row { display:flex; justify-content:space-between; gap:10px; margin-bottom:5px; }',
    '.meta-row:last-child { margin-bottom:0; }',
    '.meta-label { color:var(--muted); font-weight:800; text-transform:uppercase; }',
    '.meta-value { font-weight:900; text-align:right; }',
    '.summary { display:grid; grid-template-columns:repeat(5, 1fr); gap:8px; margin:14px 0 18px; }',
    '.box { border:1px solid var(--line); background:linear-gradient(180deg,#fff,#f8fafc); padding:10px; min-height:70px; }',
    '.label { color:var(--muted); font-size:9px; font-weight:900; letter-spacing:0.08em; text-transform:uppercase; }',
    '.value { margin-top:5px; font-size:24px; font-weight:900; letter-spacing:-0.05em; }',
    'h2 { margin:18px 0 8px; padding-bottom:4px; border-bottom:1px solid var(--line); font-size:15px; text-transform:uppercase; letter-spacing:0.06em; }',
    ...
    'th { background:#e2e8f0; color:#0f172a; font-size:9px; text-transform:uppercase; letter-spacing:0.045em; }',
    'tr:nth-child(even) td { background:#f8fafc; }',
    '.strong { font-weight:900; }',
    '.num { text-align:right; font-variant-numeric:tabular-nums; }',
    '.empty-row { text-align:center; color:var(--muted); padding:14px; }',
    '.notes-section, .narrative { margin-top:12px; border:1px solid var(--line); background:#f8fafc; padding:10px; }',
    '.note-item { padding:7px 0; border-bottom:1px solid #e2e8f0; font-size:11px; }',
    '.note-item:last-child { border-bottom:none; }',
    '.note-title { font-weight:900; margin-bottom:2px; }',
    '.narrative { font-size:12px; line-height:1.45; font-weight:700; }',
    ...
    '<div class="title-block"><h1>PRC DASH Archive Report</h1><div class="subtitle">Pfingston Reception Center Dashboard</div></div>',
    '<div class="meta">',
    '<div class="meta-row"><span class="meta-label">Week Group</span><span class="meta-value">' + escapeHtml(weekGroup) + '</span></div>',
    '<div class="meta-row"><span class="meta-label">Archived</span><span class="meta-value">' + reportDateTime(archivedAt) + '</span></div>',
    '<div class="meta-row"><span class="meta-label">Printed</span><span class="meta-value">' + escapeHtml(printedAt) + '</span></div>',
    '</div>',
    '</header>',
    '<section class="summary">',
    '<div class="box"><div class="label">Received</div><div class="value">' + totalArrived + '</div></div>',
    '<div class="box"><div class="label">Projected</div><div class="value">' + expectedTotal + '</div></div>',
    ...
    '<h2>Bus Table</h2>',
    '<table><thead><tr><th>Type</th><th>Bus #</th><th>Originating Destination</th><th>Arrived/OTW</th><th>Females</th><th>Naturalizations</th><th>Departed</th><th>Arrived</th><th>Status</th></tr></thead><tbody>' + busRows + '</tbody></table>',
    '<h2>Notes Section</h2>',
    '<section class="notes-section">' + (notesItems || '<div class="empty-row">No dorm notes recorded.</div>') + '</section>',
    '<h2>Processing Summary</h2>',
    '<section class="narrative">' + escapeHtml(processedNarrative) + '</section>',
    '<footer class="footer"><div>CUI / NO PII</div><div>Generated by PRC DASH</div></footer>',
