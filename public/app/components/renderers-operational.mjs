...
export function renderGateDialog(props = {}) {
  ...
  return `<dialog class="gate-dialog" data-gate-component="GateDialog" data-gate-overlay="dialog" data-gate-overlay-state="closed" id="${id}" aria-modal="true" aria-labelledby="${id}-title"${descriptionId ? ` aria-describedby="${descriptionId}"` : ''}><div class="gate-dialog__surface"><header><h2 id="${id}-title">${escapeHtml(props.title || 'Dialog')}</h2><button class="gate-dialog__close" type="button" data-gate-action="close" data-gate-initial-focus aria-label="Close ${escapeAttribute(props.title || 'dialog')}">×</button></header>${props.description ? `<p id="${descriptionId}">${escapeHtml(props.description)}</p>` : ''}<div class="gate-dialog__body">${props.body || ''}</div><footer>${renderActions(props.actions || [])}</footer></div></dialog>`;
}
...
export function renderGateSheet(props = {}) {
  ...
  const descriptionId = props.description ? `${id}-description` : '';
  return `<section class="gate-sheet gate-sheet--${placement}" data-gate-component="GateSheet" data-gate-overlay="sheet" data-gate-overlay-state="closed" id="${id}" role="dialog" aria-modal="true" aria-labelledby="${id}-title"${descriptionId ? ` aria-describedby="${descriptionId}"` : ''} tabindex="-1" hidden><div class="gate-sheet__surface"><header><h2 id="${id}-title">${escapeHtml(props.title || 'Sheet')}</h2><button class="gate-sheet__close" type="button" data-gate-action="close" data-gate-initial-focus aria-label="Close ${escapeAttribute(props.title || 'sheet')}">×</button></header>${props.description ? `<p id="${descriptionId}">${escapeHtml(props.description)}</p>` : ''}<div class="gate-sheet__body">${props.body || ''}</div><footer>${renderActions(props.actions || [])}</footer></div></section>`;
}
