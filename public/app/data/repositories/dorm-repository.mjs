...
export class GateDormRepository extends BaseRepository {
  ...
  async createDorm(command = {}) {
    ...
    return this.createRaw({
      ...
      dorm_name: name,
      sdq: normalizeText(command.sdq),
      section: normalizeText(command.section),
      inter_sec: normalizeText(command.interSection),
      sex: normalizeText(command.sex || 'male').toLowerCase(),
      band: command.band ? 'true' : 'false',
      ...
      overtime_sound_at: '',
      operation_id: normalizeText(command.operationId),
      display_order: toNonNegativeNumber(command.displayOrder),
      input_order: toNonNegativeNumber(command.displayOrder),
      source_row_index: toNonNegativeNumber(command.sourceRowIndex),
      dorm_identity: normalizeText(command.dormIdentity),
