// GATE's single persistence browser owner. Legacy controllers remain only for flag-off rollback.
// Never writes typed mirrors directly or registers an assumed active Week Group.
(function () {
  'use strict';
  ...
  let mode='checking',health=null,draftId=null,draftRevision=null,lastSaved='',inFlight=null,timer=null;
  let dirtyDuringBoot=false,conflict=false,actionInFlight=false,recoveredReview=null,recoveredReviewGroup='';
  ...
  function status(message,isError=false){
    ...
    if(!label){
      label=document.createElement('div');label.id='gate-draft-persistence-status';
      label.setAttribute('role','status');label.setAttribute('aria-live','polite');
      label.className='text-xs mt-2';header.appendChild(label);
    }
    label.textContent=message;label.style.color=isError?'var(--red)':'var(--text)';
  }
  async function api(action,body={}){
    const read=action==='health'||action==='draft';
    const response=await fetch(read?`${ENDPOINT}?mode=${action}`:ENDPOINT,read
    ...
    if(!response.ok||!payload.isOk){
      ...
      failure.code=payload.code||'request_failed';throw failure;
    }
    return payload;
  }
  function snapshot(){
    const group=String(byId('wg-batch-input')?.value||'').trim().toUpperCase();
    const published=byId('gate-import-total');
    const selected=document.querySelector('#gate-import-band [data-band][aria-pressed="true"]');
    // The importer owns the transient review controls. Preserve recovered D1 review values
    // until those controls are rendered again, so unrelated edits cannot erase the review.
    const saved=recoveredReviewGroup===group?recoveredReview:null;
    return {proposed_week_group:group,
      rows:(controller()?.getRows?.()||[]).map((row,index)=>({rowIndex:index,sdq:row.sdq||'',sec:row.sec||'',
        inter_sec:row.inter_sec||'',dorm_name:row.dorm_name||'',sex:row.sex||'male',band:Boolean(row.band),
        space_force:Boolean(row.space_force),load:row.load??''})),
      receiving_windows:Object.fromEntries(WINDOWS.map(key=>[key,byId(key)?.value||''])),
      import_review:{published_total:published?(published.value===''?null:published.value):(saved?.published_total??null),
        band_decision:selected?.dataset.band||saved?.band_decision||null}};
  }
  function restore(draft){
    ...
    controller()?.renderBatchGrid?.();return true;
  }
  function lockInput(){
    ...
    if(byId('init-wg-btn'))byId('init-wg-btn').disabled=locked||conflict;
    if(byId('closeout-btn'))byId('closeout-btn').disabled=mode!=='ready'||!health?.cycle||health.cycle.state!=='active'||actionInFlight;
  }
  function scheduleSave(delay=850){
    ...
    clearTimeout(timer);status('Unsaved Input changes');
  ...
  async function flushDraft(){
    ...
    clearTimeout(timer);
    if(inFlight){await inFlight;if(conflict)throw new Error('Draft conflict. Reload before proceeding.');}
    ...
    try{
      ...
      draftId=saved.draft.draft_id;draftRevision=saved.draft.revision;
      recoveredReview={...saved.draft.import_review};recoveredReviewGroup=saved.draft.proposed_week_group||'';
      ...
      if(failure.code==='draft_conflict'||failure.code==='conflict'){
        conflict=true;status('Draft conflict: another device changed this draft. Reload before editing.',true);
      }else if(!['validation','not_found'].includes(failure.code)){
        // A committed INSERT can lose its HTTP response. Retrying without a returned draft_id
        // could create a second draft. Stop automatic saves and require reconciliation.
        conflict=true;
        status('Draft save result UNKNOWN. Automatic retries stopped to avoid duplicate drafts. Preserve visible edits and verify the D1 draft before reloading.',true);
      }else status(`Input was NOT saved: ${failure.message}`,true);
      lockInput();throw failure;
  ...
  async function refreshHealth(){
    ...
    lockInput();return health;
  }
  async function refreshApp(){
    if(typeof window.refresh==='function'){
      await window.refresh();window.runGateHooks?.('afterDataChanged',{source:'gate-persistence-runtime'});
    }else window.location.reload();lockInput();
  }
  function adoptionButton(){
    ...
    const label=byId('gate-draft-persistence-status');if(!label)return;
    const button=document.createElement('button');
    button.id='gate-persistence-adopt';button.type='button';
  ...
  async function registerActive(){
    ...
    try{
      ...
      await api('adopt');await refreshHealth();
  ...
  async function initialize(){
    if(actionInFlight)return;actionInFlight=true;lockInput();
    try{
      await refreshHealth();
      ...
      try{result=await api('initialize',requested);}
        ...
        // A lost response cannot establish a failed transaction. Replay exactly the same ID/revision.
        result=await api('initialize',requested);
      }
      draftId=null;draftRevision=null;lastSaved='';recoveredReview=null;recoveredReviewGroup='';
      await refreshHealth();await refreshApp();
  ...
  async function closeout(){
    ...
    try{
      ...
      const cycleId=health.cycle.cycle_id,weekGroup=health.cycle.week_group;
      ...
      try{result=await api('closeout',{cycle_id:cycleId});}
        ...
        if(!(failure instanceof TypeError))throw failure;
        // Only transport loss is replayed, using the same cycle_id.
        result=await api('closeout',{cycle_id:cycleId});
      }
      await refreshHealth();
      if(health.cycle||health.legacy_week_group)throw new Error('An active cycle remains after closeout. Investigate before initializing.');
      ...
      recoveredReview=null;recoveredReviewGroup='';lastSaved='';controller()?.renderBatchGrid?.();
      await refreshApp();
  ...
  function lifecycleClick(event){
    ...
    // Intercept legacy handlers even before health/authentication is established.
    ...
    if(target.id==='init-wg-btn')void initialize();else void closeout();
  }
  ...
  async function boot(){
    ...
    try{
      ...
      if(draft){
        ...
        draftId=draft.draft_id;draftRevision=draft.revision;lastSaved=JSON.stringify(snapshot());
      ...
      if(dirtyDuringBoot&&!draft)scheduleSave();lockInput();adoptionButton();
    }catch(failure){
      mode='blocked';lockInput();
