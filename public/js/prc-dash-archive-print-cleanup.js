...
(function () {
  ...
  function printCleanArchiveReport() {
    ...
    const wg = clean(document.getElementById('archive-edit-wg')?.value || archive.week_group, 'week-group').replace(/[^a-z0-9_-]+/gi, '-').replace(/^-+|-+$/g, '') || 'week-group';
    const printUrl = `${window.location.origin}/print/gate-archive-report/${encodeURIComponent(wg)}`;
    ...
    printWindow.document.write(printableHtml({ archive, dorms, buses, windows: windowValues(archive) }));
