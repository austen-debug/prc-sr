...
const fixtureRecords = canonicalize(fixture.records);

test('B2-P1-F001 reproduces the historical receiving totals', () => {
  const capacity = calculateCapacityTotals(fixtureRecords, fixture.weekGroup);
  const confirmed = calculateConfirmedArrivalTotals(fixtureRecords, fixture.weekGroup);
  const active = selectActiveBuses(fixtureRecords, fixture.weekGroup);
...
test('active and malformed arrived records never enter confirmed-arrival totals', () => {
  const records = canonicalize([
    { type: 'bus', week_group: 'WG', status: 'active', otw_count: 20, arrived_at: '2026-01-01T01:00:00Z' },
    { type: 'bus', week_group: 'WG', status: 'arrived', otw_count: 30, arrived_at: '' },
    { type: 'bus', week_group: 'WG', status: 'arrived', otw_count: 40, arrived_at: '2026-01-01T02:00:00Z' }
  ]);
...
test('receiving windows are half-open and assign a boundary arrival once', () => {
  const records = canonicalize([
    ...
    { type: 'bus', week_group: 'WG', status: 'arrived', otw_count: 10, arrived_at: '2026-01-01T01:00:00Z' }
  ]);
  const windows = {
...
test('Air Force and Space Force are partitions of the same confirmed bus set', () => {
  const records = canonicalize([
    ...
    { type: 'bus', week_group: 'WG', status: 'arrived', otw_count: 44, space_force_count: 10, nat_count: 4, arrived_at: '2026-01-01T00:30:00Z' }
  ]);
  const windows = {
    receiving_day_one_start: '2026-01-01T00:00:00Z',
    receiving_day_one_end: '2026-01-01T01:00:00Z'
...
test('manifested bus totals remain distinct from confirmed arrivals', () => {
  const records = canonicalize([
    { type: 'bus', week_group: 'WG', status: 'arrived', otw_count: 40, arrived_at: '2026-01-01T00:30:00Z' },
    { type: 'bus', week_group: 'WG', status: 'active', otw_count: 20, departed_at: '2026-01-01T00:45:00Z' }
  ]);
...
test('assignment summary exposes both awaiting and over-assigned conditions', () => {
  const records = canonicalize([
    { type: 'bus', week_group: 'WG', status: 'arrived', otw_count: 50, arrived_at: '2026-01-01T00:30:00Z' },
    { type: 'dorm', week_group: 'WG', max_load: 60, current_load: 45 }
  ]);
  assert.deepEqual(calculateAssignmentSummary(records, 'WG'), {
    weekGroup: 'WG', arrived: 50, loaded: 45, awaitingAssignment: 5, overAssigned: 0
  });
});
