    ...
    return {
      ...
      async run() {
        ...
        if (normalized.startsWith('UPDATE records')) {
          ...
          if (conditional && Number(current.record_version || 0) !== Number(bound[5])) return { meta: { changes: 0 } };
          rows[index] = { ...rows[index], type: bound[0], week_group: bound[1], data: bound[2], updated_at: bound[3] };
          return { meta: { changes: 1 } };
        }
        if (normalized.startsWith('DELETE FROM records')) {
          ...
          const current = JSON.parse(rows[index].data);
          if (current.type === 'audit_event') throw new Error('audit events are append-only');
          if (normalized.includes('json_extract') && Number(current.record_version || 0) !== Number(bound[1])) return { meta: { changes: 0 } };
          rows.splice(index, 1);
          return { meta: { changes: 1 } };
        }
...
test('server assigns version and trusted role provenance on create', async () => {
  ...
  const response = await onRequestPost({
    env: { DB: db }, data: roleData('instructor'),
    request: request('POST', {
      type: 'bus', week_group: 'WG', status: 'active', record_version: 99,
      created_by_role: 'squadron', updated_by_role: 'squadron'
    })
  });
...
test('audit events are server-attributed, PII-restricted, and append-only', async () => {
  ...
  const created = await onRequestPost({
    env: { DB: db }, data: roleData('airman'),
    request: request('POST', {
      type: 'audit_event', event_type: 'dorm.load_updated', entity_type: 'dorm', entity_id: 'D1',
      actor_role: 'instructor', prior_version: 1, resulting_version: 2, metadata: { load: 40 }
    })
  });
  ...
  const prohibited = await onRequestPost({
    env: { DB: db }, data: roleData('instructor'),
    request: request('POST', {type:'audit_event',event_type:'test',entity_type:'bus',entity_id:'B1',metadata:{trainee_name:'not allowed'}})
  });
  assert.equal(prohibited.status, 400);
});

test('Squadron cannot read or mutate general records, including by direct API calls', async () => {
  const db = createD1([
  ...
  const env={DB:db};
  const read=await onRequestGet({env,data:roleData('squadron')});
  assert.equal(read.status,403);
  assert.equal(db.rows.length,3);
  for(const [method,call] of [['POST',onRequestPost],['PUT',onRequestPut],['DELETE',onRequestDelete]]) {
    const response=await call({env,data:roleData('squadron'),request:request(method,{type:'bus',__backendId:'dorm-1'})});
    assert.equal(response.status,403);
  }
  assert.equal(db.rows.length,3);
});

test('shared session verifier accepts a signed cookie and rejects absent or weak signing key', async () => {
  const env={AUTH_SECRET:'test-only-long-secret-for-squadron-cookie-check-12345'};
  const token=await issueSession('test-instructor','instructor',env);
  const request=new Request('https://gate.example/api/records',{headers:{Cookie:`prc_sr_session=${token}`}});
  const session=await verifyRequestSession(request,env);
  assert.deepEqual(session,{role:'instructor',username:'test-instructor'});
  assert.equal(await verifyRequestSession(request,{AUTH_SECRET:'test-secret'}),null);
  assert.equal(await verifyRequestSession(request,{}),null);
});
